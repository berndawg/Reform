...
internal class WhereClauseBuilder<T>(IMetadataProvider<T> metadataProvider, IDialect dialect) : ExpressionVisitor
    where T : class
{
    ...
    private void VisitMemberForColumn(Expression expression)
    {
        ...
        {
            var propertyMap = metadataProvider.GetPropertyMapByPropertyName(member.Member.Name);
            if (propertyMap != null)
            {
                _sql.Append(dialect.QuoteIdentifier(propertyMap.ColumnName));
                return;
            }

            throw new ApplicationException(
                $"The type '{metadataProvider.Type}' does not contain property metadata for the property '{member.Member.Name}'");
        }
